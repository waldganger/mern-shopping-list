...
// @desc    Delete an Item
// @access  Public
router.delete('/:id', (req, res) => {       // :id === placeholder for the id we'll pass
  Item.findById(req.params.id)              // findById vient de Mongoose et fetch l'info de l'uri
  .then(item => item.remove().then(() => res.json({delete_success: true})))
  .catch(err => res.status(404).json({delete_success: false}));
})
